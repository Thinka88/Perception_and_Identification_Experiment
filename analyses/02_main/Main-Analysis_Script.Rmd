...
```{r read_data}
data <- read_csv("results_276_XPLab+SS21+-+Group+13+-+Main_Mara,+Linus,+Cosima,+Katharina.csv")
glimpse(data)
```
...
```{r}
d_summary <- data %>% 
  mutate(language = case_when(languages == "deutsch" ~ "german",
...
There were `r sum(d_summary$gender == 'male', na.rm = TRUE)` male, `r sum(d_summary$gender == 'female', na.rm = TRUE)` female and `r sum(is.na(d_summary$gender))` participants who chose not to answer, with ages ranging from `r d_summary$age %>% unique %>% min(na.rm = TRUE)` to `r d_summary$age %>% unique %>% max(na.rm = TRUE)`. There were 10 participants in the discrimination condition and 9 in the identification condition. The first 4 participants contributed 510 data points (10 for each switch rate), the subsequent participants contributed 6 data points for each switch rate plus 10 practice trials.
There were `r sum(d_summary$language == 'german', na.rm = TRUE)` German native speakers, 1 participant who speaks German and English, 1 participant who speaks English and Arabic and `r sum(is.na(d_summary$language))` participants who did not answer this question.
...
```
Answer 1 goes in the right direction.
...
data %>% 
  ...
  summary()
```
The maximal RT is very high, there is also a big span from minimal to maximal time spent. 

We will clean the data by only selecting the main trials and the columns that are relevant for us.
We will also get rid of outliers.
The columns that are relevant for our analysis are:  
...
preprocessed_data <- data %>% 
  ...
  # change trial name to condition and switch_rate to swr
  rename(condition = trial_name) %>% 
  rename(swr = switch_rate) %>% 
  # manually include the data w/o practice trials
  filter(submission_id == 2407 | 
         submission_id == 2412 |
         submission_id == 2414 |
         submission_id == 2415 |
         # exclude practice trials
         condition == "discrimination_main" |
         condition == "identification_main"
         ) %>% 
  mutate(condition = case_when(condition == "identification_main" ~ "identification",
                               condition == "discrimination_main" ~ "discrimination",
                               TRUE ~ condition),
         outlier = case_when(RT > 8000 ~ TRUE,
                             TRUE ~ FALSE))
```
...
d
```
The accuracy for both groups, especially for the identification condition, looks off. In the Pilot part of our experiment, both conditions had a mean accuracy of over 0.8. 
We will plot each participant's responses in each switch rate to get an overview of the data.
...
```{r indiv_response_plot}
# we make swr not a factor for plotting
clean_data$swr <- as.numeric(levels(clean_data$swr))[clean_data$swr]

ggplot(clean_data, aes(x = swr, y = correct_response, color = condition)) +
  geom_point(position = position_jitter(height = 0.02), alpha = 0.2) +
  facet_wrap(vars(submission_id))
```
In our Pilot we saw that there should be more correct responses (y = 1) than incorrect responses (y = 0), especially for low or high switch rates. This makes sense because the further the switch rate is from 0.5, the easier it should be to identify which half of the shown matrix is generated at random or discriminate whether the border between the two halves is vertical or horizontal. The performance of `submission_id`s 2407, 2412, 2415, 2436, 2437, 2443, 2495, 2525, 2566, 2592 and 2594 looks quite reasonable. The performance of `submission_id`s 2414, 2440, 2506 and 2587 looks as if they misunderstood the assignment to always choose the "randomly generated" matrix, but they chose the one that did not look random. `submission_id` 2458 is the only participant in the discrimination condition whose performance looks off. In fact it looks like they confused "horizontal" and "vertical". `submission_id`s 2441, 2442 and 2488 performance leads us to think they did not at all understand the task because there is no clear pattern in their responses.  
It is definitely not ideal to work with this data, since these results are a lot different than our expectation - especially in the identification condition. But we only chose to exclude trials with too long reaction times and we had no intention to include any participant due to lack of performance because this is what we try to draw inference about. Although we can see that some people must have not read the instructions well enough or have not understood the task. It would also be wrong to invert the performance (code correct as incorrect and vice versa) of some participants where the performance looks upside down because we cannot guarantee that this performance is due to a misunderstanding of the task.  
We will suppose that the data we measured is representative of performance in such tasks and continue with our analysis as planned although the unexpectedness of the data already weakens our confidence in the results. We will discuss further problems or reasons for mistakes in the conclusion and our final paper.
...
The `Intercept`, or base level estimate for switch rate = 0 in the discrimination condition, is 1.61 (corresponding to a proportion of correct answers of approx. 0.83), the 95% Credible Interval (CI) is quite large [0.67, 2.52] but only includes positive numbers. The estimate for `conditionidentification` is -1.39 and the 95% CI ranges from [-2.74, -0.05] which is why we are confident that the proportion of correct answers is less in the identification condition. Only some levels of `swr` and the interaction terms between `conditionidentification:swr` are meaningful right now (because their CI does not include zero). We can see that the estimates for switch rates = 0.4 to switch rate = 0.66 are negative, so the proportion of correct answers goes down. This makes sense because switch rates closer to 0.5 are less easy to tell apart in both conditions from the random half. For switch rates 0.36 to 0.66 the interaction estimates are credibly above zero meaning that there were more mistakes made in the discrimination condition for these switch rates, this coincides with our observation from the data above - we recall that the performance for the identification condition sometimes looked like it was turned upside down.  
We can also see that there is substantial variability across the participants (`submission_id`) with an estimate of 1.43. This is also consistent with our observations because the performance had very high variability.

```{r pp_check, cache=TRUE}
pp_check(logistic_regression, nsamples = 100)
```
Looking at the `pp_check` we can see that the model quite neatly captures the nature of our data.
...
# extract values from the model and combine them to meaningful parameters for plotting
...
```{r plot_pred_values}
# make swr not a factor for nice plotting 
clean_data$swr <- as.numeric(levels(clean_data$swr))[clean_data$swr]
...
```
Now looking at the plot of the posterior estimates and their 95% CI, the proportion of correct answers in the discrimination condition looks similar to the one we gained in our pilot study. For lower and higher switch rates (further away from 0.5) the estimated performance is quite high (above 0.75). Then around 0.3 the proportion of correct answers goes down as expected, even below 0.5, and then goes up again until a switch rate of around 0.74 where the performance stabilizes at around 0.85.
The plot of the identification condition on the other hand does not at all look like the one from the pilot. Here we see a jagged line that stays around 0.5 which is just chance. 

### Conclusion

If the data was representative of such task, we would conclude that there is definitely a difference between the two conditions and say that the probability of correctly identifying stimuli from R and N does NOT coincide with the ease of distinguishing between the two sources. We would conclude that the identification task was a lot harder and people could only identify correctly at chance.  
But since we are not confident in the data that we obtained, we will not draw such conclusions. To investigate the hypothesis, it would be best to repeat this experiment in a controlled environment (like a lab), to be able to answer potential questions the participants might have and rule out potential distractions. It would also help to include not only practice trials, but practice trials with feedback regarding the correctness of the response. This would probably diminish obtaining data produced by misunderstanding the tasks. More elaborate criticism and propositions are in the conclusion part of our final paper.
